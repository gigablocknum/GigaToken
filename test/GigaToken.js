...
var GigaTokenMock = artifacts.require('./helpers/GigaTokenMock.sol');

contract('GigaToken', function(accounts) {

  let token;

  beforeEach(async function() {
    token = await GigaTokenMock.new(accounts[0], 100);
  });
  ...
  it('should increase total supply', async function() {
    await token.increaseSupply(100);
    let totalSupply = await token.totalSupply();
  ...
  it('should return the correct allowance amount after approval', async function() {
    let token = await GigaTokenMock.new();
  ...
  it('should return correct balances after transfer', async function() {
    let token = await GigaTokenMock.new(accounts[0], 100);
